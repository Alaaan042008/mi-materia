...
export default function HomeScreen() {
  ...
  // Actualiza el ancho de la barra mientras la canción avanza
  useEffect(() => {
    if (durationMillis > 0) {
  ...
  // PanResponder para barra interactiva
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        const newPosition = Math.min(Math.max(touchX / barLayout.width, 0), 1) * durationMillis;
        seekTo(newPosition);
      },
    })
  ).current;
      ...
      <View className="flex-1 bg-[#0b0b0b]">
        {(showSelection || !currentSong) ? (
          <View className="flex-1 px-6 pt-14">
            <Text className="text-white text-3xl font-bold mb-6 text-center">MyKast</Text>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Image source={item.image} style={{ width: 64, height: 64, borderRadius: 12 }} />
                  <View>
          ...
          <View className="flex-1 justify-center items-center px-6">
            ...
            <Image
              ...
              style={{ width: 280, height: 280, borderRadius: 24, marginBottom: 24 }}
              resizeMode="cover"
            />
            <Text className="text-white text-2xl font-bold text-center">{currentSong.title}</Text>
            <Text className="text-gray-400 text-base mt-1">{currentSong.artist}</Text>

            {/* Barra de progreso interactiva */}
            <View className="w-full mt-8">
              <View className="flex-row justify-between mb-1">
                <Text className="text-gray-400 text-xs">{formatTime(positionMillis)}</Text>
            ...
            <View className="flex-row items-center justify-center w-full mt-8 space-x-12">
              <TouchableOpacity onPress={prevSong} className="p-4">
                <Entypo name="controller-fast-backward" size={30} color="#9CA3AF" />
              </TouchableOpacity>
              <TouchableOpacity onPress={togglePlayPause} className="bg-[#FA0501] p-6 rounded-full shadow-xl">
                <FontAwesome name={isPlaying ? "pause" : "play"} size={26} color="#fff" />
              </TouchableOpacity>
            ...
            <View className="flex-row justify-around w-full mt-10 border-b border-[#333] pb-2">
              <TouchableOpacity onPress={() => setActiveTab("letra")}>
                <Text className={`text-lg ${activeTab === "letra" ? "text-[#FA0501]" : "text-gray-400"}`}>Letra</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setActiveTab("siguiente")}>
                <Text className={`text-lg ${activeTab === "siguiente" ? "text-[#FA0501]" : "text-gray-400"}`}>Siguiente</Text>
              </TouchableOpacity>
            </View>
            ...
            {activeTab === "letra" ? (
              ...
              <TouchableOpacity onPress={() => handlePlaySong(next)} className="mt-6 flex-row items-center bg-[#181818] p-3 rounded-xl border border-[#333]">
                <Image source={next.image} style={{ width: 60, height: 60, borderRadius: 12 }} />
                <View className="ml-3">
                  <Text className="text-white font-semibold">{next.title}</Text>
